...
def define_tsnet(name, num_class, cuda=True):
	if name == 'resnet20':
		...
		net = WRN.WideResNet(depth=28, widen_factor=4, num_classes=100)
	elif name == 'wrn40-2':
		net = WRN.WideResNet(depth=40, widen_factor=2, num_classes=100)
	elif name == 'wrn16-2':
		net = WRN.WideResNet(depth=16, widen_factor=2, num_classes=100)
	else:
...
class resnet20(nn.Module):
	def __init__(self, num_class):
		...
		self.CoordAtt1 = CoordAtt(16, 16)
		self.res2 = self.make_layer(resblock, 3, 16, 32)
		self.CoordAtt2 = CoordAtt(32, 32)
		self.res3 = self.make_layer(resblock, 3, 32, 64)
		self.CoordAtt3 = CoordAtt(64, 64)
	...
	def forward(self, x):
		...
		attention1 = self.CoordAtt1(rb1[1])
		rb2 = self.res2(attention1)
		attention2 = self.CoordAtt2(rb2[1])
		rb3 = self.res3(rb2[1])
		attention3 = self.CoordAtt3(rb3[1])
...
class resnet110(nn.Module):
	...
	def forward(self, x):
		...
		rb2 = self.res2(attention1)
		attention2 = self.CoordAtt2(rb2[1])
		rb3 = self.res3(attention2)
		attention3 = self.CoordAtt3(rb3[1])

		feat = self.avgpool(attention3)
		feat = feat.view(feat.size(0), -1)
		out  = self.fc(feat)
